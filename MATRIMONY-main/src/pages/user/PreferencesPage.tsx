  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              type="button"
              onClick={handleReset}
              className="px-4 py-2 rounded-xl text-xs font-extrabold bg-white/20 hover:bg-white/30 text-white border border-white/40 shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              ...
              disabled={isDeleting}
              className="px-4 py-2 rounded-xl text-xs font-extrabold bg-rose-500/25 hover:bg-rose-500/40 text-white border border-rose-400/40 shadow-sm transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Main Form Content with Framer Motion AnimatePresence */}
        ...
        {/* Right Sidebar: Live Preference Summary Glassmorphism Card */}
        <div className="lg:col-span-4 space-y-6 sticky top-24">
          <Card className="p-6 bg-gradient-to-br from-[#8B1E3F]/10 via-white to-[#D4AF37]/10 border-2 border-[#8B1E3F]/20 shadow-xl rounded-3xl space-y-5">
            <div className="flex items-center justify-between border-b border-[#8B1E3F]/20 pb-3">
              <h3 className="font-serif text-base font-extrabold text-[#8B1E3F] flex items-center gap-2">
                <Compass className="h-5 w-5 text-[#8B1E3F]" /> Live Match Filter Summary
              </h3>
              <Badge variant="gold" className="text-[10px] font-extrabold bg-[#D4AF37] text-stone-950">98% Accuracy</Badge>
            </div>

            <div className="space-y-3 text-xs divide-y divide-stone-200/60">
              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Age Window:</span>
                <span className="font-extrabold text-[#8B1E3F]">{prefs.ageMin} - {prefs.ageMax} Yrs</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Height Span:</span>
                <span className="font-bold text-stone-900">{prefs.heightMin} to {prefs.heightMax}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Religion:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{prefs.religions.join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Castes:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{prefs.castes.join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Salary Tier:</span>
                <span className="font-extrabold text-[#8B1E3F]">{prefs.incomeMin}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Education:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{prefs.educations.join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Countries:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{prefs.countries.join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">States:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{(prefs.states || []).join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Cities:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{(prefs.cities || []).join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Diet:</span>
                <span className="font-bold text-stone-900 truncate max-w-[150px]">{prefs.diet.join(', ')}</span>
              </div>

              <div className="pt-2 flex justify-between items-center">
                <span className="text-stone-500 font-semibold">Horoscope:</span>
                <span className="font-extrabold text-[#8B1E3F]">{prefs.manglik || 'Not Specified'}</span>
              </div>
            </div>

            <div className="p-3.5 bg-[#8B1E3F]/5 border border-[#8B1E3F]/20 rounded-2xl text-[11px] text-stone-700 space-y-1">
              <div className="font-extrabold flex items-center gap-1.5 text-[#8B1E3F]">
                <Heart className="h-4 w-4 text-[#8B1E3F]" /> Live Match Sync Connected
              </div>
              <p className="leading-relaxed text-stone-600">
                Filters update your AI compatibility scoring algorithm in real-time.
              </p>
            </div>
          </Card>
        </div>
