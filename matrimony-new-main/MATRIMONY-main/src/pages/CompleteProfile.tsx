...
export const CompleteProfile: React.FC = () => {
  ...
  // Detailed Profile State initialized with saved draft if present
  const [formData, setFormData] = useState<DetailedProfileRequest>(() => {
    const defaultData: DetailedProfileRequest = {
  ...
  const handleLogoutAndResumeLater = () => {
    ...
    logout();
    showToast('Progress saved! You can log in anytime to resume profile completion.');
    navigate('/login');
  };
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="bg-white/95 backdrop-blur-md p-6 sm:p-8 rounded-3xl border border-stone-200/80 shadow-none relative overflow-hidden space-y-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
            ...
            {/* Progress Meter Badge & Logout Option */}
            <div className="shrink-0 flex flex-col items-center sm:items-end gap-3">
              <div className="bg-stone-50 border border-stone-200/80 p-4 rounded-2xl text-center space-y-1.5 min-w-[160px]">
              ...
              <button
                ...
                className="text-xs font-bold text-stone-500 hover:text-[#8B1E3F] transition-colors flex items-center gap-1.5 px-3 py-1.5 rounded-xl hover:bg-stone-100/80 border border-stone-200/60"
              >
                <LogOut className="h-3.5 w-3.5" /> Save & Log Out (Resume Later)
              </button>
            </div>
          </div>
        </div>
