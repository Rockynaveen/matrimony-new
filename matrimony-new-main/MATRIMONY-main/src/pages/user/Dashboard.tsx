...
export const Dashboard: React.FC = () => {
  ...
  // Dynamic Matches List from API
  const recentMatches = (recommendations && recommendations.length > 0)
    ? recommendations.slice(0, 3).map((rec: any) => {
        const matchName = rec.full_name || rec.name || `${rec.first_name || ''} ${rec.last_name || ''}`.trim() || 'Member';
        return {
          id: String(rec.user_id || rec.id || '1'),
          name: matchName,
          verified: Boolean(rec.is_verified),
          age: rec.age || 26,
              ...
              <div className="space-y-4">
                {recentMatches.length === 0 ? (
                  <div className="py-8 text-center space-y-2">
                    ...
                    <p className="text-xs font-bold text-stone-700">No recent matches yet</p>
                    <p className="text-[11px] text-stone-400 font-medium">Explore potential matches in Search to find your perfect partner!</p>
                    <button
                      onClick={() => navigate('/search')}
                      className="mt-2 px-4 py-2 bg-[#8B1E3F] text-white text-xs font-bold rounded-xl hover:bg-[#721733] transition-colors shadow-2xs"
                    >
                      Discover Matches
                    </button>
                  </div>
